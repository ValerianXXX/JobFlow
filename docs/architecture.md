...
### Signed-update runtime — public publishing remains externally gated

Fixed per-user Windows installations include local update verification, health checks, crash-safe activation and rollback, and pinned-key verification. A visible user action opens the verifier; there is no background poller, scheduler, service, or silent download. The updater is constrained to the fixed `ValerianXXX/JobFlow` stable release endpoint, rejects unapproved redirect hosts, and requires a canonical schema-v2 manifest, pinned RSA signature, exact complete-runtime name, version, commit, size, SHA-256 digest, runtime-closure identity, layout, and public-content boundary before staging. The existing version remains active until a candidate passes its health check, and an interrupted or failed switch is recovered deterministically.

Public signed updates remain blocked until external publisher evidence and a production signature are available. The repository contains no production private key, and the local helper is not a production signer. Its optional DPAPI-protected key is restricted to development fixtures: it uses a separate storage namespace and signing envelope, must not match the pinned production public key, and is rejected by the production updater even when a caller supplies its public key. The helper never reads or migrates the former local publisher-key path and fails closed if that obsolete state is present. Before publisher evidence exists, `scripts/protected-publisher-handoff.py prepare` derives one canonical, pathless request from the exact complete-runtime archive and canonical runtime-build evidence. The request exposes only public hashes, sizes, release bindings, pinned trust identities, and the required response contract; it contains no local path, credential, or signing capability. Its `verify` stage reconstructs the request from the original bytes and fully revalidates returned publisher evidence before that evidence may enter presign preparation. The bundle builder then has a separate two-stage signature handoff: **Prepare** locks and validates the complete runtime and evidence, writes only a canonical pathless presign manifest and signing request, then stops at the protected-signature gate; **Finalize** accepts only the exact original inputs and an externally produced signature envelope, rebuilds both presign artifacts byte-for-byte, revalidates current evidence freshness and the clean candidate commit, verifies the pinned production public key, transactionally stages the exact runtime ZIP plus canonical runtime and publisher evidence under fixed public names, and commits the formal manifest/signature pair last. Clean-Windows evidence is imported only through a fixed-destination, single-link, canonical-JSON transaction that prevalidates every release and Browser Companion binding, re-runs the complete chain after the atomic replacement, and restores the previous evidence on an unexpected failure. Release readiness verifies those exact bytes and separate clean-Windows evidence before it can report ready. Local QA can never authorize or produce the production signature. No private key is included in Git, source archives, manifests, diagnostics, or command output.
...
| `%LOCALAPPDATA%\JobOps\DevelopmentFixtureSigning` | DPAPI-protected development-fixture key; domain-separated from and rejected by production updates |
