...
To assist one approved application on a supported company site, run `Install JobFlow Browser Companion.cmd` once, load the opened `browser-companion` folder as an unpacked Edge/Chrome extension, then start the per-application assist from JobFlow. On the exact approved same-page guest form, use the extension to inspect, fill and attach approved materials. JobFlow stops at `AWAITING_USER_SUBMIT`; the user must click Submit. It then observes the result and asks “Was it submitted successfully?” when evidence is unclear. It never retries automatically.

如需在受支持的公司官网辅助一项已批准申请，先运行一次 `Install JobFlow Browser Companion.cmd`，把已打开的 `browser-companion` 文件夹作为 Edge/Chrome 解压缩扩展加载，再从 JobFlow 启动逐岗位辅助。在精确获批的同页访客表单上，用扩展检查、填写并挂载获批材料。JobFlow 停在 `AWAITING_USER_SUBMIT`，必须由用户亲自点击 Submit；随后只观察结果，证据不明确时询问“是否提交成功？”，绝不自动重试。

This first real slice intentionally excludes login, CAPTCHA/MFA, cross-origin or multi-page ATS flows, account creation, email, recruiter contact and scheduling. A local approval is not a submission. / 首个真实闭环刻意不包含登录、CAPTCHA/MFA、跨域或多页 ATS、账号创建、邮件、招聘者联系与定时任务；批准本机审阅包不等于已经投递。
