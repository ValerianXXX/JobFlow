...
After the Application readiness panel has no blockers, enter one instruction such as **Handle this job for me https://company.example/careers/role** (or use the separate link field) and approve the 30-minute read-only capture. The connected AI receives the bounded JobFlow tool catalog and current redacted task state; JobFlow executes each selected high-level tool and calls the AI again at the material and form decision points. Open the role, use the JobFlow J companion to read it, click the company's Apply control yourself, then use J once more on the application form. JobFlow reads no existing input values and performs no fill, upload, or page click during capture. It prepares and verifies the job-specific materials and opens one encrypted review packet.

“自动投递准备度”没有阻挡项后，可直接输入“帮我处理这个岗位 https://公司官网岗位链接”（也可使用单独链接框），并确认 30 分钟只读授权。已连接 AI 会获得受限工具目录和当前脱敏任务状态；JobFlow 执行每个获选高层工具，并在材料与表单决策点再次调用 AI。打开岗位页后用一次 JobFlow J 读取岗位；你亲自点击公司的 Apply，进入申请表后再用一次 J。读取阶段不读取既有输入值，也不填写、上传或点击网页按钮；JobFlow 会自动生成并检查岗位材料，然后打开唯一的加密审阅包。
