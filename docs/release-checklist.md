...
- [ ] Run `scripts\protected-publisher-handoff.py prepare` against the exact complete-runtime ZIP and canonical runtime-build evidence. Transfer only the named public inputs and the emitted canonical request to the protected publisher. The request must contain no path or secret. After the protected environment returns `JobFlow-publisher-evidence.json`, run the same command with `verify`; continue only when it reports `PROTECTED_PUBLISHER_RESPONSE_VERIFIED` for the exact request, runtime evidence, and archive bytes.
