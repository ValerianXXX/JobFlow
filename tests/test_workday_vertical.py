...
class SyntheticWorkdayVerticalTests(unittest.TestCase):
    def test_saved_workday_form_handles_required_optional_and_voluntary_answers(self) -> None:
        with project_temp() as temp:
            ...
            with patch.object(socket, "socket", forbidden), patch.object(
                ...
                result = orchestrator.run_to_awaiting(
                    ...
                    route_fixture=fixtures / "synthetic-workday-route.json",
                    form_fixture=fixtures / "synthetic-workday-safe-form.html",
                    research_fixture=fixtures / "synthetic-research.html",
                    synthetic=True,
                )
                self.assertEqual(result["ats_safe_prefill"]["provider"], "workday")
                self.assertEqual(result["execution_plan"]["blockers"], [])
                service = OnboardingCenterService(PROJECT, database, onboarding)
                displayed = service.review_packet(str(result["application_id"]))
                fields = displayed["field_resolution"]["unresolved_fields"]
                self.assertEqual(len(fields), 3)
                by_class = {str(item["classification"]): item for item in fields}
                resolutions = [
                    {
                        "control_ref": by_class["work_authorization_stop"]["control_ref"],
                        "decision": "CONFIRMED_VALUE",
                        "value": "Yes",
                    },
                    {
                        "control_ref": by_class["compensation_stop"]["control_ref"],
                        "decision": "CONFIRMED_VALUE",
                        "value": "90000",
                    },
                    {
                        "control_ref": by_class["voluntary_disclosure_stop"]["control_ref"],
                        "decision": "PREFER_NOT_TO_ANSWER",
                        "value": "",
                    },
                ]
                ...
                self.assertEqual(prepared["ephemeral_field_count"], 5)
                self.assertEqual(prepared["confirmed_stop_field_count"], 2)
                self.assertEqual(prepared["skipped_optional_field_count"], 1)
                completed = lifecycle.complete_with_fresh_authorization(
                    application_id=str(result["application_id"]),
                    run_id=str(prepared["run_id"]),
                    user_confirmed=True,
                    fake_confirmation_number="SYNTHETIC-WORKDAY-RECEIPT",
                )
