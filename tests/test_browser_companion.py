...
class BrowserCompanionStaticTests(unittest.TestCase):
    ...
    def test_pairing_is_retryable_versioned_and_does_not_require_all_sites(self) -> None:
        ...
        self.assertIn('chrome.runtime.sendNativeMessage', worker)
        self.assertIn('NATIVE_HOST_NAME = "com.jobflow.browser_companion"', worker)
    ...
    def test_companion_install_helper_and_bilingual_ui_entry_are_packaged(self) -> None:
        ...
        self.assertIn("install-jobflow-native-host.ps1", helper)
        self.assertIn("chrome_web_store_url", helper)
        self.assertIn("edge_addons_url", helper)
        self.assertIn("chromewebstore.google.com", helper)
        self.assertIn("microsoftedge.microsoft.com", helper)
        self.assertIn("Get-DefaultBrowserKind", helper)
        self.assertIn("[switch]$OpenStoreOnly", helper)
        self.assertIn("JOBFLOW_BROWSER_COMPANION_STORE_CONFIG_INVALID", helper)
        self.assertIn("$existingBinding", helper)
        self.assertIn("$candidateSecretBytes.Length -eq 32", helper)
        self.assertIn("[switch]$Development", helper)
        self.assertIn("No extension folder selection is required", helper)
        ...
        self.assertIn('installationBindingPromise = null;', worker)
        self.assertIn('"BROWSER_COMPANION_BINDING_MISMATCH"', worker)
