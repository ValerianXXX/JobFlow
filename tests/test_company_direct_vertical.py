...
class SyntheticCompanyDirectVerticalTests(unittest.TestCase):
    def test_company_careers_route_reaches_review_and_verified_fake_receipt_without_transport(self) -> None:
        fixtures = PROJECT / "tests" / "fixtures"
        with project_temp() as temp:
            database = JobOpsDB(temp / "jobops.db")
            database.initialize()
            private_temp = Path(tempfile.mkdtemp(prefix="jobflow-company-private-test-"))
            ...
            with patch.object(socket, "socket", forbidden), patch.object(
                ...
                result = orchestrator.run_to_awaiting(
                    ...
                    route_fixture=fixtures / "synthetic-company-direct-route.json",
                    form_fixture=fixtures / "synthetic-company-direct-form.html",
                    research_fixture=fixtures / "synthetic-research.html",
                    synthetic=True,
                )
                self.assertEqual(result["ats_safe_prefill"]["provider"], "company")
                service = OnboardingCenterService(PROJECT, database, onboarding)
                displayed = service.review_packet(str(result["application_id"]))
                self.assertEqual(displayed["packet"]["source_route"]["route_kind"], "OFFICIAL_DIRECT")
                ...
                completed = lifecycle.complete_with_fresh_authorization(
                    ...
                    user_confirmed=True,
                    fake_confirmation_number="SYNTHETIC-COMPANY-RECEIPT",
                )
