...
class EphemeralPayloadTests(unittest.TestCase):
    ...
    def test_synthetic_private_fields_and_files_are_ephemeral_and_redacted(self) -> None:
        with project_temp() as root:
            ...
            self.assertEqual(evidence["application_answer_bundle_count"], 0)
            self.assertEqual(evidence["confirmed_stop_field_count"], 0)
            self.assertEqual(evidence["skipped_optional_field_count"], 0)
