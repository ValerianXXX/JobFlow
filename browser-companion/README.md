...
Run `Install JobFlow.cmd` from the JobFlow project root. The unified installer creates a private runtime copy under the current Windows account's Local AppData and opens its `BrowserCompanion` folder. In `edge://extensions` or `chrome://extensions`, choose **Load unpacked** and select that opened runtime folder—not this source-template directory. Confirm version `0.7.0` and fixed extension ID `hhlliaaafegldkmcgmaoaelabipcaooj`. Job discovery, page verification, AI material preparation, and post-approval page assistance continue in the local background; the popup may be closed while the home page shows persistent progress.
