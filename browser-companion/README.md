...
Run `Install JobFlow Browser Companion.cmd` from the JobFlow project root. The installer creates a private runtime copy under the current Windows account's Local AppData and opens its `BrowserCompanion` folder. In `edge://extensions` or `chrome://extensions`, choose **Load unpacked** and select that opened runtime folder—not this source-template directory. Confirm version `0.6.0` and fixed extension ID `hhlliaaafegldkmcgmaoaelabipcaooj`.
