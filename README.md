...
JobFlow（内部工程代号 JobOps）是本地优先、证据驱动的求职工作流。0.3 版从一句岗位目标开始，在可见浏览器中搜索并核验公司官网岗位，沿公司公布的 Apply 路线读取申请表，准备材料并只要求一次岗位审阅；批准后会在用户在场时连续处理多页表单，直到最终 Submit 前。登录、验证、未知问题和最终 Submit 始终交还用户；账号创建、自动重试、邮件、招聘者联系、无人值守投递和系统定时任务均不可运行。 / JobFlow 0.3 starts from one job-search instruction, verifies an official-company role in a visible browser, follows its company-published Apply route, prepares one review packet, and—after one approval—continues user-present multi-page fill and upload until the final Submit boundary.
...
| 公司官网找岗 / Company-careers discovery | 用户明确开始的可见搜索、AI 匹配与官网页实时复验；结果含糊时停下选择 / User-started visible search, AI matching, and live official-page verification; ambiguity stops for selection | 只读页面访问 / Read-only page visits |
| Greenhouse、Lever、Workday | 合成/已保存页面安全证据，不声称实时兼容 / Synthetic or saved-page safety evidence, no live claim | 0 |
| 连续处理 / Continuous processing | 一次开始后事件驱动接力、FIFO、待审批上限；无后台调度 / Event-driven continuation after one start, FIFO, bounded approvals; no unattended scheduler | 仅当前 30 分钟任务 / Current 30-minute task only |
...
最简单的 Windows 使用方式：第一次双击 `Install JobFlow.cmd`。这一入口会安装本地运行环境、准备仅位于 Local AppData 的浏览器伴侣运行目录，然后启动 JobFlow；以后只需双击 `Start JobFlow.cmd`。不需要 WSL，也不需要输入项目路径。 / On Windows, run `Install JobFlow.cmd` once. It installs the local runtime, prepares the Browser Companion under Local AppData, and starts JobFlow. Use `Start JobFlow.cmd` thereafter; WSL and manual project paths are not required.

浏览器自身仍要求一次不可绕过的安全操作：在安装器打开的 Edge/Chrome 扩展页启用 Developer mode，选择 Load unpacked，并选择同时打开的 Local AppData `BrowserCompanion` 目录（不要选择源码目录）。确认扩展版本与当前发布一致。首次开始自动搜索或访问 HTTPS 岗位页时，浏览器也可能要求用户打开一次 `J` 图标并批准 Search/HTTPS 访问；JobFlow 不会静默扩大浏览器权限。安装器生成的本机绑定证明不会进入项目、Git 或源码 ZIP。 / The browser requires one unavoidable security action: enable Developer mode, choose Load unpacked, and select the Local AppData `BrowserCompanion` folder opened by the installer—not the source tree. On first use, open the `J` icon once if the browser asks for Search/HTTPS access. JobFlow never silently broadens browser permissions. The installation-specific binding proof is excluded from the project, Git, and source archives.

普通流程从一句目标开始，例如“寻找纽约的信用风险岗位并申请”。JobFlow 在用户可见的浏览器标签页中搜索，只接受公司官网的精确岗位页，核验公司、岗位和新鲜度，再跟随该官网公开的唯一 Apply 路线进入对应申请表。它读取可见 JD 与脱敏表单结构，生成岗位专用简历、按需 Cover Letter 及已批准的 GitHub/作品集绑定，然后显示唯一审阅包。用户只批准一次当前精确包；此后浏览器伴侣会在用户在场时连续逐页填写获批字段、挂载获批材料，并只通过经重新校验的明确非最终 Next/Continue。登录、账号验证、CAPTCHA、MFA、法律/签名、敏感或未知答案会暂停并交还用户。到 `AWAITING_USER_SUBMIT` 后只有用户能点击最终 Submit；结果不明确时只询问是否成功，绝不自动重试。 / Start with one instruction such as “Find and apply to credit-risk roles in New York.” JobFlow performs a visible browser search, accepts only exact official-company job pages, verifies company, role and freshness, then follows the unique company-published Apply route. It reads visible JD text and a sanitized form structure, prepares job-specific materials, and shows one review packet. One approval binds that exact packet; the user-present companion then fills approved fields, attaches approved materials, and advances only through freshly revalidated, explicitly non-final Next/Continue controls. Login, verification, CAPTCHA, MFA, legal/signature, sensitive, or unknown questions pause for the user. Only the user can click final Submit, and ambiguous outcomes are never retried.

搜索结果不唯一、Apply 路线含糊或站点来源无法证明时，JobFlow 不会猜测；它会显示脱敏候选或要求用户接管。选错岗位可随时使用“取消本次读取并更换网址”，撤销当前授权并立即重新开始。后台准备期间主页与右下角持续显示真实阶段、已用时间和动态范围；关闭扩展小窗口不会取消任务。 / JobFlow never guesses when search results or Apply routes are ambiguous. It shows redacted candidates or requests a handoff. “Cancel this read and choose another URL” revokes the current lease and restarts immediately. During background preparation, the home page and persistent activity indicator show the current stage and elapsed-time range; closing the extension popup does not cancel the task.
...
连接完成后，模型不是一个孤立的“文本提取器”。JobFlow 会在每个决策点把同一份受限工具目录、最新脱敏任务状态及状态哈希交给 Hermes、OpenClaw 或本地模型：首次决定如何开始岗位任务；岗位读取后决定批准 Claim 的相关性和材料组合；每个表单页再理解当前控件语义。模型只返回要调用的高层 JobFlow 工具，JobFlow 主机重新校验状态、解密获批值、执行浏览器/材料动作并追加审计，然后在下一决策点再次调用模型。普通步骤采用事件驱动接力，不要求用户反复发指令；登录、验证码、未知/敏感题、审阅与最终 Submit 才暂停给用户。用户可在同一输入框直接写“帮我处理这个岗位”并粘贴公司官网链接；网址仍由主机验证，不作为模型权限。 / Once connected, the model receives the bounded JobFlow tool catalog plus a fresh redacted, hash-bound task state at every decision point. It chooses high-level tools; the JobFlow host validates, decrypts approved values, executes, audits, and calls the model again at the next decision. Ordinary stages continue event-by-event without repeated prompting, while login, verification, unknown or sensitive questions, review, and final Submit remain user gates. A single command may include the company job URL; the host, never the model, validates that URL as authority.
...
每份待审批申请还会生成一份不可静默更改的六步“自动投递步骤”：确认岗位与页面、以访客方式进入、安全字段预填、上传岗位材料、处理敏感/未知问题、用户最终提交。唯一的“批准一次并开始填写”决定同时绑定当前页面、表单、获批答案、材料和审阅包，并创建最多 30 分钟的逐岗位在场租约；不再要求第二次开始确认。浏览器伴侣完成真实预填和上传后停在 `AWAITING_USER_SUBMIT`，不包含点击最终提交的代码。用户提交后，强证据才形成确认回执；明确失败会退回审批，不确定或进程中断会进入不可自动重试的 `SUBMISSION_UNKNOWN`。 / Every packet has a tamper-evident six-step plan. The single Approve once and start filling decision binds the exact page, form, approved answers, materials, and packet to one expiring user-present lease. Fill/upload stops at `AWAITING_USER_SUBMIT`; only the user's trusted final click begins result observation, and uncertain or interrupted outcomes become non-retryable `SUBMISSION_UNKNOWN`.
...
完成“自动投递准备度”后，普通流程只需输入求职目标，也可选择直接粘贴已知的公司官网岗位链接。JobFlow 会在可见浏览器中自动搜索、核验并装填 JD 路线，跟随经证明的公司 Apply 链接读取申请表，准备岗位材料并打开唯一审阅包。批准一次后，用户在场的逐页填写与上传连续运行到最终 Submit 前；不会再要求用户重复点击岗位页、申请页或开始按钮。原来的 JD、官网页、申请表三文件入口仅保留在“高级诊断与离线 QA”。 / After readiness passes, enter a job goal or optionally paste a known official-company job URL. JobFlow visibly searches and verifies the JD route, follows the proven company Apply link, inspects the application form, prepares materials, and opens one review packet. One approval then continues user-present fill and upload until the final Submit boundary without repeated start clicks. The former three-snapshot workflow remains only under Advanced diagnostics and offline QA.
...
- `VISIBLE_OFFICIAL_JOB_SEARCH_AND_AI_MATCHING_READY`
- `ONE_REVIEW_CONTINUOUS_USER_PRESENT_ASSIST_READY`
...
- 实时找岗只在用户点击一次“开始”后运行，并受 30 分钟本机租约约束；它不会注册 Windows 任务、Codex 自动化、后台服务或无人值守调度。租约内的搜索、官网核验、Apply 路线读取、材料准备和批准后逐页辅助采用事件驱动接力；含糊结果、登录/验证、敏感或未知问题与最终 Submit 才暂停。队列达到待审批上限后按 FIFO 延后，释放名额时只在当前本机进程中继续。项目与普通 SQLite 仍只保存哈希、状态和 secure-ref，不保存私人正文。 / Live discovery runs only after one explicit Start action and is bounded by a 30-minute local lease. Search, official-page verification, Apply-route intake, preparation, and post-approval page assistance continue event by event; ambiguity, protected questions, login/verification, and final Submit remain user gates. No Windows task, Codex automation, background service, or unattended scheduler is registered.
