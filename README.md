...
每份待审批申请还会生成一份不可静默更改的六步“自动投递步骤”：实时确认岗位仍开放、以访客方式进入、安全字段预填、上传岗位材料、处理敏感/未知问题、最终提交。审阅页会用中英文直接标出每一步当前是已规划、只生成建议还是已停止，以及需要哪一种单独批准。批准审阅包本身不能进入提交状态：最终提交还必须取得另一张 1—30 分钟有效、一次性、与当前执行计划、路线、表单、材料、审阅包和实时新鲜度证据绑定的明确授权；提交开始时两张授权在同一事务内消费，内容变化、过期或重放都会失败关闭。隔离执行控制器会把计划校验、新鲜度绑定、零修改预填验证、最终确认、假提交和回执验证写成仅含哈希的追加式检查点；缺失可靠回执会进入不可自动重试的 `SUBMISSION_UNKNOWN`。若进程在最终授权消费后的任一检查点中断，恢复器只会根据已持久化的申请状态与可靠回执收敛为 `CONFIRMED` 或 `SUBMISSION_UNKNOWN`，自动重试次数固定为 0。某一岗位停在用户面前时，队列仍会继续处理其他岗位，直到达到用户设置的待审批数量上限。当前只实现本机/合成门禁，不注册实时传输。 / Every review packet includes a hash-bound six-step execution plan. Review approval alone cannot submit: a second short-lived, one-time final authorization must bind the current plan, route, form, uploads, packet, and freshness evidence, and both approvals are consumed atomically. The isolated controller records hash-only append-only checkpoints; interruption recovery converges only to confirmed-with-receipt or non-retryable unknown, with zero automatic retries. Live transport remains absent.

未来外部动作不会共用一个模糊的总授权：实时岗位读取、申请表检查、安全预填和材料上传分别使用限时、逐动作、单次会话；系统会先完整核验所需范围，再按官网读取、表单检查、可选预填、上传的固定顺序消费，避免执行到一半才发现缺少授权。最终提交、账号、邮件和调度不能混入这类会话。首页安全面板显示外部动作总开关，并预留可立即使全部会话失效的双语急停按钮。当前构建的总开关默认关闭且无法启用生产动作。 / Future live read, inspection, optional prefill, and upload use separate expiring one-use scopes. The complete scope is preflighted before any action is consumed. Final submit, account, messaging, and scheduling cannot be folded into them. The bilingual dashboard exposes a global emergency stop; production activation remains unavailable and off by default.
...
Company、Greenhouse、Lever 与 Workday 现在共用一份提供商中立的传输契约：官网读取、表单检查、预填、上传、最终提交和回执验证具有固定顺序；每个动作只接收申请/路线/表单/计划/请求与授权哈希，不允许私人原值或文件正文进入传输信封。最终提交只能绑定短时一次性最终授权，回执缺失时禁止猜测和自动重试。当前所有提供商均明确标记 `live_transport_registered=false`。另有一个仅限合成测试的临时载荷桥，用于证明加密字段可只在内存中解析、材料可在项目外短暂解密并在返回前完整清理；生产启用仍不可用。 / Company, Greenhouse, Lever and Workday share one provider-neutral, hash-only transport contract. The envelope forbids raw private values and file content, binds each action to its exact authorization type, and explicitly reports that no live transport is registered. A synthetic-only ephemeral payload proof resolves encrypted fields in process, stages materials outside the project and confirms cleanup before returning; production activation remains unavailable.
...
- 连续接入当前采用 `MANUAL_TICK_ONLY`：`plan-continuous-intake` 只计算本次本地批次与剩余容量，首次 `run-queue` 必须由用户明确运行。批次既可使用合成测试件，也可使用已经完成入职的真实 Candidate Profile 与逐岗位明确选择的本地 JD、官网页、申请表和研究快照；真实资料模式缺少任一证据就整项失败关闭。一个岗位的本地材料错误不会阻止后续岗位继续准备；达到待审批上限后，剩余项按 FIFO 延后。延后项只保存哈希绑定的 secure-ref 与项目内相对路径；用户通过界面或 CLI 审批、退回或拒绝一项释放名额时，JobFlow 会在同一本机进程中自动准备下一项，缺失原始描述时才要求重新运行原批次。结果只返回序号、状态、申请 ID 和错误代码，不返回 secure-ref、文件路径或私人正文。它不会注册 Windows 任务、Codex 自动化、后台服务、浏览器动作或网络动作。 / Continuous intake starts only from an explicit local tick. Hash-bound saved descriptors let a review decision fill the newly freed slot in the same local process, while redacted outcomes, FIFO fairness and zero background, browser, network or scheduler actions remain enforced.
