...
Company、Greenhouse、Lever 与 Workday 现在共用一份提供商中立的传输契约：官网读取、表单检查、预填、上传、最终提交和回执验证具有固定顺序；每个动作只接收申请/路线/表单/计划/请求与授权哈希，不允许私人原值或文件正文进入传输信封。最终提交只能绑定短时一次性最终授权，回执缺失时禁止猜测和自动重试。当前所有提供商均明确标记 `live_transport_registered=false`。 / Company, Greenhouse, Lever and Workday share one provider-neutral, hash-only transport contract. The envelope forbids raw private values and file content, binds each action to its exact authorization type, and explicitly reports that no live transport is registered.
