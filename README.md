...
JobFlow（内部工程代号 JobOps）是本地优先、证据驱动的求职工作流。它把岗位资料处理到可审阅的 `AWAITING_APPROVAL`，然后继续处理其他岗位，直到用户设置的待审批上限。当前最小真实闭环支持用户在场、逐岗位批准的公司官网单页访客申请：读取当前表单、填写已批准字段、挂载已批准材料，并严格停在最终提交按钮前。最终提交、账号创建、邮件、招聘者联系、无人值守投递和系统定时任务均不可运行。
...
| 公司官网单页访客预填与材料上传 / Same-page guest company application assist | 用户在场、逐岗位一次性授权、浏览器伴侣 / User-present, one-use per-application approval, Browser Companion | 仅获批字段与材料 / Approved fields and materials only |
...
若要使用真实公司官网的预填与材料上传，还需在每台电脑上做一次浏览器伴侣安装：双击 `Install JobFlow Browser Companion.cmd`，在打开的 Edge/Chrome 扩展页开启“开发人员模式”，选择“加载解压缩的扩展”，再选择脚本已为你打开的 `browser-companion` 文件夹。浏览器伴侣只连接本机 JobFlow，不读取 Cookie、Token、页面正文或未获批输入值，也没有任何提交函数。 / For real company-site fill and material attachment, run `Install JobFlow Browser Companion.cmd` once per computer, enable Developer mode, choose Load unpacked, and select the opened `browser-companion` folder. The companion talks only to local JobFlow, never reads cookies, tokens, page bodies or unapproved values, and contains no submit implementation.

真实使用路径：先在 JobFlow 中完成资料与 Claim 审批，选择一项已批准申请并点击“开始公司官网辅助”；在浏览器中打开该申请绑定的精确 HTTPS 地址，点击扩展图标并选择检查/填写。JobFlow 只接受同一页面、同一来源、访客模式且无 CAPTCHA、MFA、登录、跨域表单或 iframe 的表单；验证通过后真实填写字段并挂载文件，然后显示 `AWAITING_USER_SUBMIT`。此时必须由用户亲自点击网页的 Submit。浏览器伴侣只观察随后出现的成功、失败或不确定结果；不确定时 JobFlow 询问“是否提交成功？”，无论答案如何都绝不自动重试。 / Complete profile and Claim approval first, start assistance for one approved application, open its exact HTTPS URL, then use the companion to inspect and fill. Supported pages are same-page, same-origin guest forms without CAPTCHA, MFA, login, cross-origin forms or iframes. JobFlow fills and attaches approved data, stops at `AWAITING_USER_SUBMIT`, observes the result after the user's own Submit click, and never retries automatically.
...
每个岗位的材料都从同一份已批准、不可变的 Master Resume 生成岗位专用副本；通过入职页接入的 DOCX 简历会被明确登记为加密母版，PDF 只作为不可编辑参考。确认一条 Claim 并不等于允许它出现在申请材料中：完成资料后，用户还需在“自动投递准备度”区域一次性批准当前精确措辞用于简历、求职信和申请回答；授权与母版哈希、Profile、当前资料版本和 Claim 文字绑定，任何修改都会使旧授权失效。普通 DOCX 不要求用户手工插入占位符：JobFlow 会把 AI 已确认 Claim 映射回原简历段落，展示原文、建议类别与建议选择；用户一次确认后，只允许岗位副本在这些哈希绑定的位置改写，母版始终不变。建议位置与类别加密保存，不在项目中保存原文。只有表单明确包含 Cover Letter 上传字段时，才按已授权 Claim 与有来源的公司信息临时生成求职信。GitHub、作品集 URL 与作品集文件也只在表单存在对应字段时绑定。获批材料可以由浏览器伴侣真实挂载到受支持的公司官网单页表单；最终提交始终由用户完成。 / Every job-specific resume is derived from the same immutable approved master. Confirmed Claims require a separate hash-bound applicant authorization before material generation; revisions invalidate it. Cover letters and GitHub/portfolio bindings are created only for matching form fields. The Browser Companion can attach approved materials to a supported same-page company form, while final submit always remains the user's action.
...
Company、Greenhouse、Lever 与 Workday 继续共用提供商中立、仅哈希的离线传输契约；其中只有 `browser_companion` 注册了真实传输，而且范围严格限于用户在场的公司官网同页访客表单检查、预填和材料挂载。私人值仅在本机内存中解析，文件以一次性字节流传给固定扩展，普通数据库和日志只保存哈希与动作结果。该能力没有宣称已兼容任意实时 Greenhouse、Lever 或 Workday 页面，最终提交仍不存在。 / The provider-neutral offline contract remains shared across Company, Greenhouse, Lever and Workday. The only registered real transport is `browser_companion`, strictly limited to user-present same-page guest company-form inspection, fill and attachment. Private values are resolved in local memory and files use one-use byte streams; no arbitrary live ATS compatibility or final-submit capability is claimed.
...
用户可通过 `secure-onboard-resume` 从经授权的 Downloads 范围安全接入简历，并由系统建立 Candidate Profile、Answer Bank 和 Claim 审阅草稿。只有 PDF 时会明确保留 `EDITABLE_MASTER_DOCX_MISSING`，仍需补充可编辑 DOCX 才能做版式保持的母版修改。所有个人 Claim 都要逐条审阅。使用 Browser Companion 前，还必须准备并批准具体申请、在每项申请上授予一次用户在场辅助授权，并自行确认目标页面符合站点条款。当前真实范围只包含同页公司官网访客表单的检查、预填与材料上传；登录、多页 ATS、最终提交和无人值守运行仍不在范围内。
