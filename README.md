...
每个岗位的材料都从同一份已批准、不可变的 Master Resume 生成岗位专用副本；系统在进入申请审批队列前完成 DOCX/PDF 生成、原始分辨率渲染和结构 QA，绝不改写母版。只有本地表单快照明确包含 Cover Letter 上传字段时，才按已批准 Claim 与有来源的公司信息临时生成求职信。GitHub、作品集 URL 与作品集文件也只在表单存在对应字段时绑定：公开链接在普通审阅包中仅显示哈希和绑定状态，文件仅显示 secure-ref、哈希与安全文件名。所有实际上传和最终提交仍保持关闭。 / Every job-specific resume is derived from the same immutable approved master before review; Cover Letters and portfolio/GitHub bindings are created only when matching form fields exist, and every real upload remains blocked.
...
- 主界面的“解析已保存的公司招聘页 / Parse a saved company careers page”可读取用户选择的本地 HTML/JSON 快照；快照不会写入项目、数据库或待审批队列，页面代码也不会执行。`discover-official-jobs` 可提出公司官网、Workday、Greenhouse 或 Lever 岗位候选，但不联网、不确认岗位仍开放，也不把候选视为已验证路线；每条结果都必须在以后取得单独授权后重新做实时新鲜度与路线验证。 / The main UI can parse a user-selected local HTML/JSON careers-page snapshot without retaining it, executing page code, or mutating the application queue. Every candidate remains pending a separately authorized live freshness and route check.
