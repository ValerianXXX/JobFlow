...
每个岗位的材料都从同一份已批准、不可变的 Master Resume 生成岗位专用副本；通过入职页接入的 DOCX 简历会被明确登记为加密母版，PDF 只作为不可编辑参考。确认一条 Claim 并不等于允许它出现在申请材料中：完成资料后，用户还需在“自动投递准备度”区域一次性批准当前精确措辞用于简历、求职信和申请回答；授权与母版哈希、Profile、当前资料版本和 Claim 文字绑定，任何修改都会使旧授权失效。普通 DOCX 若没有 JobFlow 的明确安全替换位置，会显示“需要建立安全改写位置”，不会假装已经能保持版式自动改写。只有本地表单快照明确包含 Cover Letter 上传字段时，才按已授权 Claim 与有来源的公司信息临时生成求职信。GitHub、作品集 URL 与作品集文件也只在表单存在对应字段时绑定：公开链接在普通审阅包中仅显示哈希和绑定状态，文件仅显示 secure-ref、哈希与安全文件名。所有实际上传和最终提交仍保持关闭。 / Every job-specific resume is derived from the same immutable approved master. Confirmed Claims require a separate hash-bound applicant authorization before material generation; revisions invalidate it. An ordinary DOCX without explicit safe tailoring positions is reported as needing template preparation rather than being silently rewritten. Cover Letters and portfolio/GitHub bindings are created only when matching form fields exist, and every real upload remains blocked.
