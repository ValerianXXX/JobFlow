...
const STRINGS = {
  zh: {
    ...
    browserAssistNotApproved: "这份申请尚未完成审阅批准，请先批准当前审阅包。", browserAssistReviewUpgrade: "这份申请由旧版 JobFlow 审阅。请重新读取当前申请表并完成一次新版审阅后再开始；旧批准不会被静默沿用。", browserAssistRouteUnsupported: "该页面不属于已批准的公司官网、Greenhouse、Lever 或 Workday 路线，JobFlow 已安全停止。", browserAssistActive: "已有一份申请正在辅助处理中，请先完成或点击紧急停止。", browserAssistWrongPage: "当前页面不在已批准路线内，或表单在审批后发生了不安全变化。JobFlow 已停止。", browserAssistSafetyStop: "页面需要登录、注册、验证码、MFA 或其他人工步骤。请亲自完成后再从浏览器伴侣恢复；JobFlow 不读取凭据。", browserAssistUnsupportedControl: "表单包含当前无法安全自动填写的控件；JobFlow 没有修改页面。", browserAssistUploadMismatch: "已批准材料与当前上传控件无法一一对应；JobFlow 没有上传。", browserAssistFinalLocked: "最终 Submit 在代码层锁定，只能由你亲自点击。", browserAssistLeaseInvalid: "本次 30 分钟授权已失效，请从该申请重新开始。",
    intakeControlTitle: "用户在场的连续处理", intakeControlBody: "保存本机处理节奏；到时间后仍需你点击“立即处理”。JobFlow 不注册 Windows 计划任务，也不会在后台访问网站。", intakeControlBoundary: "授权窗口只允许处理已经安全保存的本机待办，不授权浏览器、网络、上传或最终提交。", intakeIntervalLabel: "提醒间隔（分钟）", intakeAuthorizationLabel: "本次授权有效（小时）", configureIntakeControl: "保存处理节奏", pauseIntakeControl: "暂停接收新岗位", resumeIntakeControl: "恢复接收新岗位", runLocalWake: "立即处理本机待办", intakeStatusNotConfigured: "尚未设置", intakeStatusReady: "等待你操作", intakeStatusDue: "可以立即处理", intakeStatusPaused: "已暂停", intakeStatusExpired: "授权已到期", intakeControlMetaEmpty: "设置后仍需你亲自点击运行；不会创建后台任务。", intakeControlMetaReady: "下次建议处理：{next} · 本次授权至：{until}", intakeControlMetaPaused: "新岗位接收与本机待办处理已暂停。", intakeControlMetaExpired: "本次处理授权已到期；请重新保存处理节奏。", intakeControlSaved: "本机处理节奏已保存；没有创建后台任务。", intakeControlPaused: "新岗位接收与本机待办处理已暂停。", intakeControlResumed: "已恢复接收新岗位。", intakeWakeComplete: "本机待办已处理；没有打开网站或执行外部动作。", configuringIntakeControl: "正在保存本机处理节奏…", pausingIntakeControl: "正在暂停新岗位接收…", resumingIntakeControl: "正在恢复新岗位接收…", runningLocalWake: "正在处理已安全保存的本机待办…", intakeControlInvalid: "请使用有效的提醒间隔（5–1440 分钟）和授权窗口（1–168 小时）。", intakeControlNotConfigured: "请先保存本机处理节奏。", intakeControlExpired: "本次本机处理授权已到期，请重新保存处理节奏。", intakePausedError: "新岗位接收已暂停；请先点击“恢复接收新岗位”。",
    ...
    packetFieldTitle: "先确认这个岗位的专属问题", packetFieldBody: "这些答案只用于当前岗位，并立即保存为本机 DPAPI 加密引用。审阅包、项目文件和普通数据库不会保存明文。", packetFieldSeparateHelp: "上传、页面导航、账号动作和最终提交由各自的授权门控制，不会被误算成尚未回答。", packetFieldRequired: "必答", packetFieldOptional: "选答", packetFieldDecision: "处理方式", packetFieldValue: "本岗位答案", packetFieldConfirmValue: "填写并确认", packetFieldUseGeneratedNarrative: "使用已审阅的岗位求职信", packetFieldPreviewNarrative: "预览本岗位生成稿", packetFieldClosePreview: "关闭并清除预览", packetFieldNarrativePreviewTitle: "仅在当前本机会话显示", loadingNarrativePreview: "正在解密并核验本岗位生成稿…", packetFieldPreferNot: "不愿回答", packetFieldNotApplicable: "不适用", packetFieldValuePlaceholder: "输入这个岗位要求的准确答案", packetFieldConsent: "我确认这些是当前岗位的准确答案，并同意仅在本机加密保存。", savePacketFields: "加密保存并生成新版审阅包", savingApplicationFields: "正在加密岗位专属答案并重新绑定审阅包…", packetFieldsSaved: "岗位专属答案已加密；请审阅新版审阅包。", packetFieldsRequired: "先完成所有标出的岗位专属问题，才能批准审阅包。", packetFieldInvalid: "有一个岗位专属答案不完整或不属于当前表单，请检查后重试。", packetFieldUnknowns: "这个条件在岗位页面中没有明确答案。你可以确认已知晓该不确定性；JobFlow 不会把它改写成事实。", packetFieldAcknowledgeUnknown: "知晓并保留为未知",
  ...
  en: {
    ...
    browserAssistNotApproved: "This application has not been approved yet. Approve the current review packet first.", browserAssistReviewUpgrade: "This application was reviewed by an older JobFlow version. Read the current form and complete one fresh review before starting; the old approval will not be silently reused.", browserAssistRouteUnsupported: "This page is outside the approved company, Greenhouse, Lever, or Workday route, so JobFlow stopped safely.", browserAssistActive: "Another application is already in an assisted session. Finish it or use the emergency stop first.", browserAssistWrongPage: "This page is outside the approved route, or the form changed unsafely after review. JobFlow stopped.", browserAssistSafetyStop: "The page needs login, registration, CAPTCHA, MFA, or another human step. Complete it yourself and resume from the companion; JobFlow never reads credentials.", browserAssistUnsupportedControl: "The form contains a control that cannot yet be filled safely. JobFlow made no page changes.", browserAssistUploadMismatch: "The approved materials do not map one-to-one to the current upload controls. Nothing was uploaded.", browserAssistFinalLocked: "Final Submit is locked in code and only you can click it.", browserAssistLeaseInvalid: "This 30-minute authorization expired. Start again from the application.",
    intakeControlTitle: "User-present continuous processing", intakeControlBody: "Save a local processing cadence; when it is due, you still click Run now. JobFlow does not register a Windows task or visit websites in the background.", intakeControlBoundary: "The authorization window covers only already-saved local work. It does not authorize browser, network, upload, or final-submit actions.", intakeIntervalLabel: "Reminder interval (minutes)", intakeAuthorizationLabel: "Authorization window (hours)", configureIntakeControl: "Save processing cadence", pauseIntakeControl: "Pause new job intake", resumeIntakeControl: "Resume new job intake", runLocalWake: "Process local queue now", intakeStatusNotConfigured: "Not configured", intakeStatusReady: "Waiting for you", intakeStatusDue: "Ready to run", intakeStatusPaused: "Paused", intakeStatusExpired: "Authorization expired", intakeControlMetaEmpty: "After setup, every run still needs your click; no background task is created.", intakeControlMetaReady: "Next suggested run: {next} · authorized until: {until}", intakeControlMetaPaused: "New intake and local deferred processing are paused.", intakeControlMetaExpired: "This local processing authorization expired. Save the cadence again.", intakeControlSaved: "Local processing cadence saved; no background task was created.", intakeControlPaused: "New intake and local deferred processing are paused.", intakeControlResumed: "New job intake resumed.", intakeWakeComplete: "Local deferred work processed without opening websites or performing external actions.", configuringIntakeControl: "Saving the local processing cadence…", pausingIntakeControl: "Pausing new job intake…", resumingIntakeControl: "Resuming new job intake…", runningLocalWake: "Processing already-saved local deferred work…", intakeControlInvalid: "Use a valid interval (5–1440 minutes) and authorization window (1–168 hours).", intakeControlNotConfigured: "Save a local processing cadence first.", intakeControlExpired: "This local processing authorization expired; save the cadence again.", intakePausedError: "New job intake is paused. Click Resume new job intake first.",
...
const LOCAL_ERROR_KEYS = {
  ...
  BROWSER_REVIEW_UPGRADE_REQUIRED:"browserAssistReviewUpgrade",
...
function renderGuidedIntake(){
  ...
  const intakePaused=state.data?.dashboard?.intake_control?.status==="PAUSED";
  ...
  start.disabled=demo||!ready||assistActive||intakePaused||!companionReady||(retryPairing?false:(!hasGoal||(active&&!terminal&&!restartPairing)));
  ...
  message.textContent=intakePaused?t("intakePausedError"):assistActive?t("companionSessionActive"):ready?[statusMessage,availabilityNotice,connectionNotice].filter(Boolean).join(" "):t("guidedReadinessRequired");
...
function browserAssistApplyFailureMessage(result){
  ...
  const reasons=state.locale==="en"?{
    ...
    COMPANION_ARIA_COMBOBOX_CHANGED:"the searchable menu structure changed",
    COMPANION_ARIA_COMBOBOX_OPTION_NOT_FOUND:"the searchable menu no longer offers the approved choice; its prior value was restored",
    ...
    COMPANION_ARIA_COMBOBOX_CHANGED:"可搜索下拉框的结构已经变化",
    COMPANION_ARIA_COMBOBOX_OPTION_NOT_FOUND:"可搜索下拉框没有审阅时批准的选项，原值已恢复",
...
function intakeControlStatusLabel(status){
  const key={
    NOT_CONFIGURED:"intakeStatusNotConfigured",READY:"intakeStatusReady",DUE:"intakeStatusDue",
    PAUSED:"intakeStatusPaused",AUTHORIZATION_EXPIRED:"intakeStatusExpired"
  }[status]||"intakeStatusNotConfigured";
  return t(key);
}

function intakeControlTime(value){
  if(!value)return "—";
  const parsed=new Date(value);
  return Number.isNaN(parsed.getTime())?"—":parsed.toLocaleString(state.locale==="zh"?"zh-CN":"en-US",{dateStyle:"short",timeStyle:"short"});
}

function renderIntakeControl(control){
  const value=control||{status:"NOT_CONFIGURED",configured:false,manual_run_allowed:false,paused:false};
  const status=document.querySelector("#intakeControlStatus"), meta=document.querySelector("#intakeControlMeta");
  if(!status||!meta)return;
  status.textContent=intakeControlStatusLabel(value.status);
  status.classList.toggle("unsafe",value.status==="PAUSED"||value.status==="AUTHORIZATION_EXPIRED");
  const interval=document.querySelector("#intakeIntervalMinutes"), hours=document.querySelector("#intakeAuthorizationHours");
  if(document.activeElement!==interval&&value.interval_minutes)interval.value=String(value.interval_minutes);
  if(document.activeElement!==hours&&value.configured&&value.authorized_until&&value.updated_at){
    const delta=Math.max(1,Math.round((new Date(value.authorized_until).getTime()-new Date(value.updated_at).getTime())/3600000));
    if(Number.isFinite(delta))hours.value=String(Math.min(168,delta));
  }
  meta.textContent=value.status==="PAUSED"?t("intakeControlMetaPaused"):
    value.status==="AUTHORIZATION_EXPIRED"?t("intakeControlMetaExpired"):
    value.configured?t("intakeControlMetaReady").replace("{next}",intakeControlTime(value.next_user_run_at)).replace("{until}",intakeControlTime(value.authorized_until)):
    t("intakeControlMetaEmpty");
  document.querySelector("#runLocalWake").disabled=value.manual_run_allowed!==true;
  document.querySelector("#pauseIntakeControl").classList.toggle("hidden",value.paused===true);
...
function applicationFieldDecisionOptions(field){
  ...
  const options=[
    ...
    ["USE_GENERATED_NARRATIVE","packetFieldUseGeneratedNarrative"],
...
function applicationFieldValueControl(field){
  ...
  const requestedMax=Number(field.max_characters),maxCharacters=Number.isInteger(requestedMax)&&requestedMax>0&&requestedMax<=4000?requestedMax:4000;
  if(options.length){
  ...
  if(String(field.control_type||"").toLowerCase()==="textarea"){
    return `<textarea class="application-field-value" maxlength="${maxCharacters}" autocomplete="off" placeholder="${escapeHtml(t("packetFieldValuePlaceholder"))}"></textarea>`;
  }
  return `<input class="application-field-value" type="text" maxlength="${maxCharacters}" autocomplete="off" placeholder="${escapeHtml(t("packetFieldValuePlaceholder"))}">`;
}
const APPLICATION_NARRATIVE_PREVIEW_TIMEOUT_MS=120000;
let applicationNarrativePreviewTimer=null;
function clearApplicationNarrativePreviews(root=document){
  if(applicationNarrativePreviewTimer!==null){window.clearTimeout(applicationNarrativePreviewTimer);applicationNarrativePreviewTimer=null;}
  root.querySelectorAll(".application-narrative-preview").forEach(preview=>{
    const text=preview.querySelector("pre");if(text)text.textContent="";
...
function renderApplicationFieldResolution(result){
  ...
  const fieldRows=fields.map(field=>`<article class="application-field-row" data-control-ref="${escapeHtml(field.control_ref)}">
    ...
    <div class="application-field-answer"><label><span>${escapeHtml(t("packetFieldValue"))}</span>${applicationFieldValueControl(field)}</label>${field.generated_narrative_available?`<div class="application-narrative-tools"><button class="text-action preview-application-narrative" type="button">${escapeHtml(t("packetFieldPreviewNarrative"))}</button><div class="application-narrative-preview hidden"><strong>${escapeHtml(t("packetFieldNarrativePreviewTitle"))}</strong><pre></pre><button class="text-action close-application-narrative-preview" type="button">${escapeHtml(t("packetFieldClosePreview"))}</button></div></div>`:""}</div>
  </article>`).join("");
...
document.addEventListener("click", async event => {
  ...
  if(configureIntake){
    ...
    if(!Number.isInteger(interval)||interval<5||interval>1440||!Number.isInteger(hours)||hours<1||hours>168){showToast(t("intakeControlInvalid"),true);return;}
    try{
      await withActivity("configuringIntakeControl",async()=>{
        await api("intake-control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"CONFIGURE",interval_minutes:interval,authorization_hours:hours,user_confirmed:true})});
        await refreshLatest();
      });
      showToast(t("intakeControlSaved"));
    }catch(error){handleUiError(error);}
    return;
  }
  const pauseIntake=event.target.closest("#pauseIntakeControl"), resumeIntake=event.target.closest("#resumeIntakeControl");
  if(pauseIntake||resumeIntake){
    const action=pauseIntake?"PAUSE":"RESUME", activity=pauseIntake?"pausingIntakeControl":"resumingIntakeControl";
    try{
      await withActivity(activity,async()=>{
        await api("intake-control",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action,user_confirmed:true})});
        await refreshLatest();
      });
      showToast(t(pauseIntake?"intakeControlPaused":"intakeControlResumed"));
    }catch(error){handleUiError(error);}
    return;
  }
  const runLocalWake=event.target.closest("#runLocalWake");
  if(runLocalWake){
    try{
      await withActivity("runningLocalWake",async()=>{
        await api("run-local-wake",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({user_confirmed:true})});
        await refreshLatest();
      },{estimatedSeconds:45});
      showToast(t("intakeWakeComplete"));
    }catch(error){handleUiError(error);}
    return;
  }
  ...
  const previewNarrative=event.target.closest(".preview-application-narrative");
  if(previewNarrative){
    const packet=state.reviewPacket?.packet,row=previewNarrative.closest(".application-field-row");
    if(!packet||!row){showToast(t("reviewPacketUnavailable"),true);return;}
    try{
      let result=null;
      await withActivity("loadingNarrativePreview",async()=>{
        result=await api("application-narrative-preview",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({
          application_id:state.reviewPacket.application_id,expected_packet_hash:packet.content_hash,control_ref:row.dataset.controlRef
        })});
      });
      clearApplicationNarrativePreviews(document.querySelector("#applicationFieldResolutionList"));
      const preview=row.querySelector(".application-narrative-preview"),text=preview?.querySelector("pre");
      if(!preview||!text||typeof result?.narrative!=="string")throw Object.assign(new Error(t("reviewPacketUnavailable")),{code:"APPLICATION_NARRATIVE_INVALID"});
      text.textContent=result.narrative;result.narrative="";preview.classList.remove("hidden");
      applicationNarrativePreviewTimer=window.setTimeout(()=>clearApplicationNarrativePreviews(row),APPLICATION_NARRATIVE_PREVIEW_TIMEOUT_MS);
    }catch(error){handleUiError(error);}
    return;
  }
  const closeNarrativePreview=event.target.closest(".close-application-narrative-preview");
  if(closeNarrativePreview){clearApplicationNarrativePreviews(closeNarrativePreview.closest(".application-field-row")||document);return;}
  const closePacket=event.target.closest("#closeReviewPacket");
  if(closePacket){clearApplicationNarrativePreviews();state.reviewPacket=null;state.reviewDecision="";document.querySelector("#reviewPacketBody").replaceChildren();document.querySelector("#applicationFieldResolutionList").replaceChildren();document.querySelector("#applicationFieldResolutionPanel").classList.add("hidden");document.querySelector("#reviewPacketPanel").classList.add("hidden");renderWorkflowNow();return;}
...
document.addEventListener("change",event=>{
  ...
  if(event.target.matches('input[name="packetDecision"]')){clearApplicationNarrativePreviews();state.reviewDecision=event.target.value;updatePacketDecisionButton();}
  if(event.target.matches(".application-field-decision")){clearApplicationNarrativePreviews(event.target.closest(".application-field-row")||document);updateApplicationFieldResolutionButton();}
  else if(event.target.matches("#applicationFieldResolutionConfirm,.application-field-value")){updateApplicationFieldResolutionButton();}
...
window.addEventListener("pagehide",()=>clearApplicationNarrativePreviews());
document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="hidden")clearApplicationNarrativePreviews();});
