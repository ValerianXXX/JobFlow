...
function Normalize-InstalledRecords([string]$BuildRoot, [string]$AppRoot) {
    $program = @'
import base64
import csv
import hashlib
...
records = sorted(
    root.rglob("*.dist-info/RECORD"),
    key=lambda item: item.relative_to(root).as_posix(),
)
if not records:
    raise SystemExit("INSTALLED_RECORDS_MISSING")
...
for record in records:
    ...
    for row in source_rows:
        if len(row) != 3:
            raise SystemExit("INSTALLED_RECORD_ROW_INVALID")
        name = row[0]
        ...
        if (
            ...
            raise SystemExit("INSTALLED_RECORD_PATH_INVALID")
        seen.add(name)

        if ".." in pure.parts:
            # pip console launchers are installed outside the bounded target
            # tree and are deliberately removed.  Their generated bytes embed
            # the temporary install path, so their stale RECORD rows must not
            # survive in the deterministic portable runtime.
            if not any(part.casefold() in {"bin", "scripts"} for part in pure.parts):
        ...
        normalized.append((name, "sha256=" + digest, str(len(payload))))

    if not self_seen:
        raise SystemExit("INSTALLED_RECORD_SELF_MISSING")
    output = io.StringIO(newline="")
    writer = csv.writer(output, lineterminator="\n")
    writer.writerows(sorted(normalized, key=lambda row: row[0]))
...
'@
    ...
    ) { throw "JOBFLOW_RUNTIME_INSTALLED_RECORD_NORMALIZATION_FAILED" }
}
...
function Install-OfflineApplication([string]$BuildRoot, [string]$AppRoot, [object[]]$RuntimePackages, [object]$Application) {
    ...
    Normalize-InstalledRecords $BuildRoot $AppRoot
}
