...
function Get-TrustedWindowsPowerShell {
    ...
    if ([string]$signature.Status -cne "Valid" -or $null -eq $signature.SignerCertificate -or [string]$signature.SignerCertificate.Subject -notmatch '(^|,\s*)O=Microsoft Corporation(,|$)') { throw "JOBFLOW_TRUSTED_WINDOWS_POWERSHELL_REQUIRED" }
    return $candidate
}
...
if (-not ("JobFlowInstalledDiscoveryFiles" -as [type])) {
    Add-Type -TypeDefinition @'
using System.IO; using System.Runtime.InteropServices; using Microsoft.Win32.SafeHandles;
public static class JobFlowInstalledDiscoveryFiles {
...
function Open-DiscoverySingleLinkFile([string]$Path) {
    Assert-LocalPath $Path
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) { throw "JOBFLOW_DISCOVERY_RUNTIME_INVALID" }
    $stream = $null
    try {
        $stream = New-Object IO.FileStream($Path, [IO.FileMode]::Open, [IO.FileAccess]::Read, [IO.FileShare]::Read)
        $info = New-Object JobFlowInstalledDiscoveryFiles+Info
        if (-not [JobFlowInstalledDiscoveryFiles]::GetFileInformationByHandle($stream.SafeFileHandle, [ref]$info) -or [long]$info.LinkCount -ne 1 -or
            ([IO.FileAttributes]$info.Attributes -band ([IO.FileAttributes]::Directory -bor [IO.FileAttributes]::Device -bor [IO.FileAttributes]::ReparsePoint)) -ne 0) { throw "JOBFLOW_DISCOVERY_RUNTIME_INVALID" }
        return $stream
    }
    catch { if ($null -ne $stream) { $stream.Dispose() }; throw "JOBFLOW_DISCOVERY_RUNTIME_INVALID" }
}
function Read-DiscoveryPointer([string]$Path) {
    $stream = Open-DiscoverySingleLinkFile $Path
    try {
        if ($stream.Length -le 0 -or $stream.Length -gt 65536) { throw "JOBFLOW_DISCOVERY_POINTER_INVALID" }
        $bytes = New-Object byte[] ([int]$stream.Length); $offset = 0
        while ($offset -lt $bytes.Length) { $read = $stream.Read($bytes, $offset, $bytes.Length - $offset); if ($read -le 0) { throw "JOBFLOW_DISCOVERY_POINTER_INVALID" }; $offset += $read }
        $value = ([Text.Encoding]::UTF8.GetString($bytes).TrimStart([char]0xFEFF)) | ConvertFrom-Json
        $actual = @($value.PSObject.Properties.Name | Sort-Object)
        if ($value.schema_version -isnot [int] -and $value.schema_version -isnot [long]) { throw "JOBFLOW_DISCOVERY_POINTER_INVALID" }
        ...
        if ([int64]$value.schema_version -ne 2 -or @(Compare-Object $expected $actual -SyncWindow 0).Count -ne 0 -or $value.product -isnot [string] -or [string]$value.product -cne "JobFlow" -or $value.version_directory -isnot [string] -or [string]$value.version_directory -cnotmatch '^v[0-9]+\.[0-9]+\.[0-9]+-[0-9a-f]{12}$' -or $value.version -isnot [string] -or [string]$value.version -cnotmatch '^[0-9]+\.[0-9]+\.[0-9]+$' -or $value.source_commit -isnot [string] -or [string]$value.source_commit -cnotmatch '^[0-9a-f]{40}$' -or $value.source_payload_sha256 -isnot [string] -or [string]$value.source_payload_sha256 -cnotmatch '^sha256:[0-9a-f]{64}$' -or $value.runtime_closure_manifest_sha256 -isnot [string] -or [string]$value.runtime_closure_manifest_sha256 -cnotmatch '^sha256:[0-9a-f]{64}$' -or $value.runtime_tree_sha256 -isnot [string] -or [string]$value.runtime_tree_sha256 -cnotmatch '^sha256:[0-9a-f]{64}$' -or $value.release_key_id -isnot [string] -or [string]$value.release_key_id -cne 'sha256:1037057f8578a60ac5b3dc030cb2d70ad945ec3b5fb51fa3944fcafa77146339' -or $value.bootstrap_version -isnot [string] -or [string]$value.bootstrap_version -cnotmatch '^[0-9]+\.[0-9]+\.[0-9]+$' -or $value.platform -isnot [string] -or [string]$value.platform -cne "windows-x64") { throw "JOBFLOW_DISCOVERY_POINTER_INVALID" }
        $payloadHex = ([string]$value.source_payload_sha256).Substring(7)
        if ([string]$value.version_directory -cne ("v" + [string]$value.version + "-" + $payloadHex.Substring(0, 12))) { throw "JOBFLOW_DISCOVERY_POINTER_INVALID" }
        return @{ Value = $value; Lock = $stream }
    }
    catch { $stream.Dispose(); throw "JOBFLOW_DISCOVERY_POINTER_INVALID" }
}
function ConvertTo-DiscoveryCanonicalJson([object]$Value) {
    ...
    if ($Value -is [PSCustomObject]) {
        ...
        $members = foreach ($property in $properties) { (ConvertTo-Json ([string]$property.Name) -Compress) + ":" + (ConvertTo-DiscoveryCanonicalJson $property.Value) }
        return "{" + [string]::Join(",", [string[]]$members) + "}"
    }
    if ($Value -is [Collections.IEnumerable]) {
        $items = foreach ($item in $Value) { ConvertTo-DiscoveryCanonicalJson $item }
        return "[" + [string]::Join(",", [string[]]$items) + "]"
    }
    throw "JOBFLOW_DISCOVERY_POINTER_INVALID"
}
function Get-DiscoveryCanonicalSha256([object]$Value) {
    $bytes = [Text.UTF8Encoding]::new($false).GetBytes((ConvertTo-DiscoveryCanonicalJson $Value))
    $hasher = [Security.Cryptography.SHA256]::Create()
    try { $raw = $hasher.ComputeHash($bytes) } finally { $hasher.Dispose() }
    return "sha256:" + (-join ($raw | ForEach-Object { $_.ToString("x2") }))
}
function Invoke-DiscoveryInstalledTrustVerification {
    $bootstrapPath = Join-Path $PSScriptRoot "jobflow-bootstrap.ps1"
    Assert-LocalPath $bootstrapPath
    try { (Open-DiscoverySingleLinkFile $bootstrapPath).Dispose() } catch { throw "JOBFLOW_BOOTSTRAP_TRUST_ROOT_INVALID" }
...
try { $verifiedInstallation = Invoke-DiscoveryInstalledTrustVerification }
catch { exit 2 }

$discoveryLockPath = Join-Path $localRoot "Data\state\.authorized-discovery-task.lock"
Assert-LocalPath $discoveryLockPath
$runtimeLockPath = Join-Path $localRoot "Data\state\.jobflow-runtime-maintenance.lock"
Assert-LocalPath $runtimeLockPath
try {
...
try {
    ...
    Assert-LocalPath $rollbackTransactionPath
    Assert-LocalPath $rollbackTransactionBackupPath
    ...
    try { $pointerRecord = Read-DiscoveryPointer $pointerPath }
    catch { exit 2 }
    $pointerFileLock = $pointerRecord.Lock
    $pointer = $pointerRecord.Value
    if ((Get-DiscoveryCanonicalSha256 $pointer) -cne [string]$verifiedInstallation.pointer_sha256) { exit 2 }
    ...
    $pythonPath = Join-Path $versionRoot "runtime\python.exe"
    ...
    try { $pythonFileLock = Open-DiscoverySingleLinkFile $pythonPath }
    catch { exit 2 }
    try { (Open-DiscoverySingleLinkFile $versionMarkerPath).Dispose(); (Open-DiscoverySingleLinkFile $dataMarkerPath).Dispose() }
    catch { exit 2 }
